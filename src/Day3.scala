object Day3 {
  ...
  def part2(input: String): Unit = {
    ...
    val gear = lines.zipWithIndex.map {
      case (group, idx) =>
        val regexGear = """\*""".r
        ...
        val thisRow = anyGears.flatMap { gear =>
          ...
          if (up2.nonEmpty) {
            ...
            val ones = List(
              if (up1.nonEmpty) {
                // on same row
                Some(List(getNumberAtIndex(gear - 1, first), getNumberAtIndex(gear + 1, first)).flatten.distinct)
              } else None,
              if (here1.nonEmpty) {
                // on same row
                println(second)
                val x =
                  Some(List(getNumberAtIndex(gear - 1, second), getNumberAtIndex(gear + 1, second)).flatten.distinct)
                println(x)
                x
              } else None,
              if (below1.nonEmpty) {
                // on same row
                Some(List(getNumberAtIndex(gear - 1, third), getNumberAtIndex(gear + 1, third)).flatten.distinct)
              } else None
            ).flatten
            println("ones " + ones)
            val cleanOnes =
              if (ones.length == 2) ones.map(_.headOption)
              else if (ones.length == 1 && ones.head.length == 2){
                ones.head.map(a => Some(a))
              }
              else List.empty
            println("clean " + cleanOnes)

            cleanOnes.map(_.headOption)
          }
        }
        println(s"${idx + 2} " + thisRow)
        thisRow.grouped(2).map(_.flatten.product).toList.sum
    }
    println("Gears = " + gear)
    println("Gears = " + gear.sum)
// 10060059 too low 78720868 too low 86742923 high 84155715 84159075
